...
class HistoryService:
    ...
    @staticmethod
    def _get_interval_config(interval: str, now_local: datetime) -> Tuple[datetime, timedelta, Callable[[datetime], str], str]:
        ...
        """
        if interval == "1min":
            return (now_local - timedelta(minutes=15), timedelta(minutes=1), lambda dt: dt.strftime("%H:%M"), '%Y-%m-%d %H:%M')
        elif interval == "15min":
            return (now_local - timedelta(hours=4), timedelta(minutes=15), lambda dt: f"{dt.hour:02d}:{(dt.minute // 15) * 15:02d}", '%Y-%m-%d %H:%M')
        elif interval == "hour":
            return (now_local - timedelta(hours=24), timedelta(hours=1), lambda dt: dt.strftime("%H:00"), '%Y-%m-%d %H:00')
        elif interval == "day":
            return (now_local - timedelta(days=7), timedelta(days=1), lambda dt: dt.strftime("%d.%m"), '%Y-%m-%d')
        elif interval == "week":
            def get_week_label(dt: datetime) -> str:
                monday = dt - timedelta(days=dt.weekday())
                sunday = monday + timedelta(days=6)
                return f"{monday.strftime('%d.%m')}-{sunday.strftime('%d.%m')}"
            return (now_local - timedelta(weeks=4), timedelta(days=7), get_week_label, '%Y-%m-%d')
        elif interval == "month":
            return (now_local - timedelta(days=365), timedelta(days=20), lambda dt: dt.strftime("%m.%Y"), '%Y-%m')
        elif interval == "year":
            return (now_local - timedelta(days=365 * 5), timedelta(days=100), lambda dt: dt.strftime("%Y"), '%Y')
        else:
            raise HTTPException(status_code=400, detail="Invalid interval specified")

    @staticmethod
    def get_historical_data(interval: str, obj_type: str, db: Session) -> list[HistoricalTrafficData]:
        """Витягує та агрегує історичні дані трафіку для графіка.

        Використовує SQL GROUP BY для оптимізації пам'яті (замість вивантаження всіх записів).

        Args:
            interval (str): Часовий інтервал угруповання.
            obj_type (str): Тип об'єкта для фільтрації ('all', 'person', 'transport').
            db (Session): Сесія SQLAlchemy.

        Returns:
            list[HistoricalTrafficData]: Список агрегованих даних для побудови графіка на фронтенді.
        """
        tz_local = ZoneInfo(AppConfig.TIMEZONE)
        now_local = datetime.now(tz_local)
        
        start_time_local, step, get_bucket_key, sql_fmt = HistoryService._get_interval_config(interval, now_local)
        ...
        for bucket_str, state, count in db_results:
            ...
            dt_utc = datetime.strptime(bucket_str, parse_fmt).replace(tzinfo=ZoneInfo("UTC"))
            dt_local = dt_utc.astimezone(tz_local)
            time_key = get_bucket_key(dt_local)
            
            if time_key in grouped_data:
                grouped_data[time_key]["total"] += count
                grouped_data[time_key]["states"][state] = grouped_data[time_key]["states"].get(state, 0) + count
        ...
        for time_key in labels:
            ...
            dominant = max(states, key=states.get) if states else TrafficStatus.FREE
            state_str = dominant.value if isinstance(dominant, TrafficStatus) else dominant
    ...
    def get_current_buckets(now: datetime) -> dict:
        ...
        Використовується WebSocket-броадкастером для реалізації патерну Backend-Driven UI.
