...
def fetch_initial_stats_from_db():
    ...
    try:
        with SessionLocal() as db:
            ...
            initial_persons = db.query(TrafficLog).filter(TrafficLog.object_class == ObjectType.PERSON).count()
            initial_transport = initial_total - initial_persons
            
            one_min_ago = datetime.now(ZoneInfo("UTC")) - timedelta(seconds=TrafficConfig.INTENSITY_WINDOW_SEC)
            recent_logs = db.query(TrafficLog).filter(TrafficLog.timestamp >= one_min_ago).all()
            crossing_timestamps = [log.timestamp.timestamp() for log in recent_logs]
