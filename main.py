...
def process_video_stream(
    ...
    try:
        while True:
            ...
            frame_input = (
                ImageUtils.apply_clahe(frame) if Config.USE_CLAHE else frame
            )
...
def main() -> None:
    """Оркестратор: Ініціалізує компоненти,
    підключає камеру та запускає цикл."""
    logger.info("Ініціалізація системи...")

    # Ініціалізація лічильника та трекера трафіку
    counter = LineCounter(
        polygon=Config.ROAD_POLYGON, line_position=Config.LINE_POSITION
    )
    traffic_state = TrafficStateTracker(
    ...
    try:
        with SSHCameraController(
            Config.RPI_IP, Config.RPI_USER, Config.RPI_PASS
        ) as camera:
