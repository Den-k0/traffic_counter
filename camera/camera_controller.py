...
class SSHCameraController:
    ...
    def start_stream(self) -> bool:
        """Спроба підключитися по SSH і запустити
        `rpicam-vid` на віддаленому хості.
        ...
        for attempt in range(1, self.max_retries + 1):
            try:
                ...
                cmd = (
                    ...
                    f"tcp://0.0.0.0:{Config.STREAM_PORT} >/dev/null 2>&1 &"
                )
                self.ssh.exec_command(cmd)

                logger.info("Очікування підняття порту...")
                wait_cmd = (
                    f"timeout {Config.STREAM_TIMEOUT} bash -c "
                    f"'while ! ss -ltn | grep -q :{Config.STREAM_PORT}; "
